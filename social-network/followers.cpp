...
followers::followers(Graph* graph , string uname ,QWidget *parent) :
    QDialog(parent),
    ui(new Ui::followers)
{
    ui->setupUi(this);
...
void followers::on_ok_clicked()
{
    this->close();
}

void followers::widgetListView()
{
    ...
    vector<string> vusers = DataBase::getFollowers(uname);
    ...
    connect(listWidget, &QListWidget::itemClicked, this, &followers::profilePicClicked);
}

void followers::profilePicClicked(QListWidgetItem* item)
{
    if (!item)
    {
        qDebug() << "Item is null!";
